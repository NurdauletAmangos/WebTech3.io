function qs(sel, root=document){ return root.querySelector(sel); }
function qsa(sel, root=document){ return Array.from(root.querySelectorAll(sel)); }

document.addEventListener('DOMContentLoaded', function() {
    const loginForm = document.getElementById('loginForm');
    if (!loginForm) return;

    const submitBtn = loginForm.querySelector('button[type="submit"]');
    const emailInput = loginForm.querySelector('input[type="email"]');
    const passwordInput = loginForm.querySelector('input[type="password"]');


    function showNotification(message, type = "info") {
        document.querySelectorAll(".notification").forEach(el => el.remove());

        const notification = document.createElement("div");
        notification.className = `notification notification-qs{type}`;
        notification.textContent = message;
        document.body.appendChild(notification);

        setTimeout(() => notification.classList.add("show"), 100);
        setTimeout(() => {
            notification.classList.remove("show");
            setTimeout(() => notification.remove(), 300);
        }, 3000);
    }

    // Ошибки под полем
    function showFieldError(input, message) {
        const error = document.createElement("div");
        error.className = "error-message text-danger mt-1";
        error.textContent = message;
        input.insertAdjacentElement("afterend", error);
    }

    //Основная логика валидации + логина
    loginForm.addEventListener('submit', async function(e) {
        e.preventDefault();

        // Удаляем предыдущие ошибки
        loginForm.querySelectorAll(".error-message").forEach(el => el.remove());

        let valid = true;
        const emailValue = emailInput.value.trim();
        const passwordValue = passwordInput.value.trim();

        // Проверка email
        const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+qs/;
        if (emailValue === "") {
            showFieldError(emailInput, "Введите логин (email).");
            valid = false;
        } else if (!emailPattern.test(emailValue)) {
            showFieldError(emailInput, "Некорректный формат email.");
            valid = false;
        }

        // Проверка пароля
        if (passwordValue === "") {
            showFieldError(passwordInput, "Введите пароль.");
            valid = false;
        } else if (passwordValue.length < 6) {
            showFieldError(passwordInput, "Пароль должен быть не менее 6 символов.");
            valid = false;
        }

        if (!valid) {
            showNotification("", "error");
            return;
        }

        // Имитация входа
        const originalText = submitBtn.textContent;
        submitBtn.innerHTML = '<div class="spinner"></div> Пожалуйста, подождите...';
        submitBtn.disabled = true;

        try {
            await new Promise(resolve => setTimeout(resolve, 2000));

            showNotification('', 'success');
            setTimeout(() => {
                submitBtn.textContent = originalText;
                submitBtn.disabled = false;
                window.location.href = "home.html";
            }, 1500);
        } catch (error) {
            showNotification('Ошибка входа. Попробуйте снова.', 'error');
            submitBtn.textContent = originalText;
            submitBtn.disabled = false;
        }
    });

    const openIdBtn = document.querySelector('.login-openid-btn');
    if (openIdBtn) {
        openIdBtn.addEventListener('click', function() {
            showNotification('Перенаправление на провайдера OpenID...', 'info');
        });
    }
});


function initAccordion(){
    qsa(".accordion-header").forEach(h=>{
        h.addEventListener("click", ()=>{
            h.classList.toggle("active");
            const c = h.nextElementSibling;
            if(!c) return;
            if(c.style.maxHeight){ c.style.maxHeight=null; c.style.paddingTop="0"; }
            else { c.style.maxHeight=c.scrollHeight+"px"; c.style.paddingTop="10px"; }
        });
    });
}












// === Настройки ===
const API_KEY = "AIzaSyDdwCfhn1-HR4Lm1OLMoKv1bcGIvlZQSmo"; // вставь сюда свой Google API Key
const CALENDAR_ID = "primary";     // можно заменить на конкретный ID календаря

// Массив дней недели на русском
const WEEK_DAYS = ["Воскресенье","Понедельник","Вторник","Среда","Четверг","Пятница","Суббота"];

// Загружаем клиент Google API
function loadGoogleCalendar() {
  gapi.load("client", initClient);
}

// Инициализация Google API клиента
function initClient() {
  gapi.client.init({
    apiKey: API_KEY,
    discoveryDocs: ["https://www.googleapis.com/discovery/v1/apis/calendar/v3/rest"]
  }).then(() => {
    return gapi.client.calendar.events.list({
      calendarId: CALENDAR_ID,
      timeMin: (new Date()).toISOString(), // только будущие события
      showDeleted: false,
      singleEvents: true,
      orderBy: "startTime",
      maxResults: 50
    });
  }).then(response => {
    const events = response.result.items;
    renderEvents(events);
  }).catch(err => console.error(err));
}

// Отображаем события в таблице
function renderEvents(events) {
  const table = document.querySelector(".calendar table");
  const tbody = table.querySelector("tbody");
  
  events.forEach(event => {
    const startStr = event.start.dateTime || event.start.date;
    const startDate = new Date(startStr);
    const dayName = WEEK_DAYS[startDate.getDay()]; // день недели на русском
    const startHour = startDate.getHours();
    const endDate = new Date(event.end.dateTime || event.end.date);
    const endHour = endDate.getHours();

    // Проходим по строкам таблицы
    tbody.querySelectorAll("tr").forEach(row => {
      const timeText = row.cells[0].textContent.trim(); // формат "08:00 - 10:00"
      const [rowStart, rowEnd] = timeText.split(" - ").map(t => parseInt(t.split(":")[0]));

      // Проверяем, совпадает ли время события с ячейкой
      if(startHour >= rowStart && startHour < rowEnd) {
        // Находим правильный столбец дня
        let colIndex = -1;
        table.querySelectorAll("thead th").forEach((th, idx) => {
          if(th.textContent.trim() === dayName) colIndex = idx;
        });

        if(colIndex > 0) {
          const cell = row.cells[colIndex];
          const eventHTML = `<b>${event.summary}</b>`;
          cell.innerHTML = cell.innerHTML ? cell.innerHTML + "<br>" + eventHTML : eventHTML;
        }
      }
    });
  });
}

// Запуск при загрузке страницы
window.addEventListener("load", loadGoogleCalendar);


















function initThemeToggle() {
  const btn = document.querySelector("#themeToggle");
  if (!btn) return;

  const root = document.documentElement;

  // 1️⃣ Проверяем сохранённое состояние темы
  const savedTheme = localStorage.getItem("theme");
  if (savedTheme === "dark") {
    root.classList.add("dark");
    btn.textContent = "Светлый режим";
  } else {
    root.classList.remove("dark");
    btn.textContent = "Темный режим";
  }

  // 2️⃣ Переключаем при клике и сохраняем выбор
  btn.addEventListener("click", () => {
    const isDark = root.classList.toggle("dark");
    localStorage.setItem("theme", isDark ? "dark" : "light");
    btn.textContent = isDark ? "Светлый режим" : "Темный режим";
  });
}


function initShowTime() {
  const btn = qs("#showTime");
  const out = qs("#liveTime");
  if (!btn || !out) return;

  let timerId = null;

  const updateClock = () => {
    const now = new Date();
    out.textContent = now.toLocaleTimeString("ru-RU", { hour12: false });
  };

  // При нажатии на кнопку — показываем часы и убираем кнопку
  btn.addEventListener("click", () => {
    btn.style.display = "none";
    out.style.display = "inline-block";
    updateClock();
    timerId = setInterval(updateClock, 1000);
    playBeep?.();
  });

  // При нажатии на часы — возвращаем кнопку и останавливаем таймер
  out.addEventListener("click", () => {
    clearInterval(timerId);
    out.style.display = "none";
    btn.style.display = "inline-block";
    playBeep?.();
  });

  out.style.display = "none";
}



function initReadMore(){
    const text = qs("#readmore-text");
    const btn = qs("#readmore-btn");
    if(!text || !btn) return;
    btn.addEventListener("click", ()=>{
        const collapsed = text.getAttribute("data-collapsed")==="true";
        text.setAttribute("data-collapsed", collapsed?"false":"true");
        text.style.display = collapsed ? "block" : "-webkit-box";
        btn.textContent = collapsed ? "Скрыть" : "Показать больше";
    });
    text.style.display = "-webkit-box";
    text.style.webkitLineClamp = "2";
    text.style.webkitBoxOrient = "vertical";
    text.style.overflow = "hidden";
}

function initStars(){
    const stars = qsa(".star");
    const label = qs("#ratingText");
    if(stars.length===0 || !label) return;
    let current = 0;
    function setRating(n){
        current = n;
        stars.forEach((s,i)=>{
            const active = i < n;
            s.classList.toggle("active", active);
            s.setAttribute("aria-checked", String(active));
        });
        label.textContent = n===0 ? "Нет оценки" : `Оценка: qs{n}/5`;
    }
    stars.forEach((s,idx)=>{
        s.addEventListener("click", ()=>{ setRating(idx+1); playBeep(); });
    });
    setRating(0);
}

function initGalleryHero(){
    const hero = qs("#hero");
    const thumbs = qs("#thumbs");
    if(!hero || !thumbs) return;
    thumbs.addEventListener("click", (e)=>{
        const img = e.target.closest("img");
        if(!img) return;
        hero.src = img.src;
        hero.alt = "Просмотр: " + (img.alt||"");
        hero.style.transform = "scale(0.98)";
        setTimeout(()=>hero.style.transform="scale(1)", 120);
    });
}

function initMenuKeyboardNav(){
    const menu = qs("#topMenu");
    if(!menu) return;
    const items = qsa('[role="menuitem"]', menu);
    if(items.length===0) return;
    menu.addEventListener("keydown", (e)=>{
        if(!["ArrowRight","ArrowLeft","Home","End"].includes(e.key)) return;
        e.preventDefault();
        const idx = items.indexOf(document.activeElement);
        switch(e.key){
            case "ArrowRight": items[(idx+1+items.length)%items.length].focus(); break;
            case "ArrowLeft": items[(idx-1+items.length)%items.length].focus(); break;
            case "Home": items[0].focus(); break;
            case "End": items[items.length-1].focus(); break;
        }
    });
}

function initLoadMore(){
    const btn = qs("#loadMore");
    const container = qs("#courseCards");
    if(!btn || !container) return;
    const data = [
        {img:"data:image/png;base64,", title:"JS Async", text:"Промисы, async/await"},
        {img:"data:image/jpeg;base64,", title:"Web Audio", text:"Звук в браузере"}
    ];
    btn.addEventListener("click", async ()=>{
        const subset = data.filter((_,i)=>i!==1).map(renderCourseCard);
        subset.forEach(el=>container.appendChild(el));
        btn.disabled = true;
        btn.textContent = "Готово";
        playBeep();
    });
}

function renderCourseCard(item){
    const a = document.createElement("article");
    a.className = "card card--equal";
    a.setAttribute("role","listitem");
    a.innerHTML = `
    <div class="card__media"><img src="qs{item.img}" alt=""></div>
    <div class="card__content"><h3>qs{item.title}</h3><p>qs{item.text}</p></div>
    <p class="card__actions"><a class="btn" href="#">Подробнее</a></p>`;
    return a;
}

function initSoundOnAction(){
    const btn = qs("#soundBtn");
    if(!btn) return;
    btn.addEventListener("click", (e)=>{ e.preventDefault(); playBeep(); });
}

function playBeep(){
    const ctx = new (window.AudioContext||window.webkitAudioContext)();
    const o = ctx.createOscillator();
    const g = ctx.createGain();
    o.type = "sine";
    o.frequency.value = 880;
    o.connect(g);
    g.connect(ctx.destination);
    g.gain.setValueAtTime(0.0001, ctx.currentTime);
    g.gain.exponentialRampToValueAtTime(0.2, ctx.currentTime+0.02);
    g.gain.exponentialRampToValueAtTime(0.0001, ctx.currentTime+0.15);
    o.start();
    o.stop(ctx.currentTime+0.16);
}



function showFieldError(input, message){
    const error = document.createElement("div");
    error.className = "error-message";
    error.textContent = message;
    input.insertAdjacentElement("afterend", error);
}

function initBackgroundChanger(){
    const btn = qs("#background_changer_course");
    if(!btn) return;
    btn.addEventListener("click", ()=>{
        const colors = ["#FFB6B9", "#1d3d75", "#1239d4", "#e46e1f", "#92dd19", "#1717aa"];
        const randomColor = colors[Math.floor(Math.random()*colors.length)];
        document.body.style.backgroundColor = randomColor;
    });
}

function initClockTicker(){
    const clock = qs("#Clock");
    if(!clock) return;
    function updateClock(){
        const now = new Date();
        const hh = String(now.getHours()).padStart(2,"0");
        const mm = String(now.getMinutes()).padStart(2,"0");
        const ss = String(now.getSeconds()).padStart(2,"0");
        clock.textContent = `qs{hh}:qs{mm}:qs{ss}`;
    }
    updateClock();
    setInterval(updateClock, 1000);
}





$(document).ready(function () {
  const $searchInput = $("#search-input, input[type='search']");
  const $searchBtn = $("#search-btn, button[type='submit']");
  const $suggestions = $("#suggestions");
  const $searchForm = $("form.search");
  const currentPage = window.location.pathname.split("/").pop();

  // === 1️⃣  Универсальная логика: переход на courses.html, если мы НЕ на ней ===
  function redirectToCourses(query) {
    if (!query) return;
    window.location.href = `courses.html?query=${encodeURIComponent(query)}`;
  }

  // === 2️⃣  Поиск и фильтрация курсов (только если мы на courses.html) ===
  function applySearch(query) {
    const $courses = $(".course-item");
    if ($courses.length === 0) return; // если курсов нет — пропускаем

    $suggestions.empty();

    if (!query) {
      $suggestions.hide();
      removeHighlights();
      $courses.show();
      return;
    }

    let matches = [];
    $courses.each(function () {
      const title = $(this).find(".course-title").text().toLowerCase();
      if (title.includes(query)) matches.push(title);
    });

    if (matches.length > 0) {
      matches.slice(0, 5).forEach(match => {
        $suggestions.append(`<li>${match}</li>`);
      });
      $suggestions.show();
    } else {
      $suggestions.hide();
    }

    $courses.each(function () {
      const title = $(this).find(".course-title").text().toLowerCase();
      $(this).toggle(title.includes(query));
    });

    highlightMatches(query);
  }

  // === 3️⃣  Сортировка видимых курсов ===
  function sortVisibleCourses() {
    const $container = $(".course-item").first().parent();
    if ($container.length === 0) return;

    const $visible = $container.find(".course-item:visible");
    $visible.sort(function (a, b) {
      const at = $(a).find(".course-title").text().trim().toLowerCase();
      const bt = $(b).find(".course-title").text().trim().toLowerCase();
      return at.localeCompare(bt);
    });

    $visible.each(function () {
      $container.append(this);
    });
  }

  // === 4️⃣  Подсветка совпадений ===
  function highlightMatches(query) {
    removeHighlights();
    if (!query) return;
    const regex = new RegExp(`(${escapeRegExp(query)})`, "gi");
    $(".course-title a").each(function () {
      const originalText = $(this).text();
      const newText = originalText.replace(regex, '<span class="highlight">$1</span>');
      $(this).html(newText);
    });
  }

  function removeHighlights() {
    $(".course-title a").each(function () {
      $(this).html($(this).text());
    });
  }

  function escapeRegExp(string) {
    return string.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  }

  // === 5️⃣  Позиционирование подсказок ===
  function positionSuggestions() {
    const inputOffset = $searchInput.offset();
    const inputHeight = $searchInput.outerHeight();
    if (!inputOffset) return;
    $suggestions.css({
      position: "absolute",
      top: inputOffset.top + inputHeight + 5,
      left: inputOffset.left,
      width: $searchInput.outerWidth(),
      zIndex: 1000
    });
  }

  // === 6️⃣  Поведение при вводе ===
  $searchInput.on("input", function () {
    const query = $(this).val().toLowerCase();

    if (currentPage === "courses.html") {
      applySearch(query);
      positionSuggestions();
    }
  });

  // === 7️⃣  При нажатии Enter ===
  $searchInput.on("keydown", function (e) {
    if (e.key === "Enter") {
      e.preventDefault();
      const query = $(this).val().trim().toLowerCase();

      if (currentPage === "courses.html") {
        applySearch(query);
        sortVisibleCourses();
      } else {
        redirectToCourses(query);
      }
    }
  });

  // === 8️⃣  При клике на кнопку ===
  $searchBtn.on("click", function (e) {
    e.preventDefault();
    const query = $searchInput.val().trim().toLowerCase();

    if (currentPage === "courses.html") {
      applySearch(query);
      sortVisibleCourses();
    } else {
      redirectToCourses(query);
    }
  });

  // === 9️⃣  Подсказки и клик вне ===
  $suggestions.on("click", "li", function () {
    const selected = $(this).text();
    $searchInput.val(selected);
    $suggestions.hide();
    if (currentPage === "courses.html") {
      applySearch(selected.toLowerCase());
      sortVisibleCourses();
    } else {
      redirectToCourses(selected.toLowerCase());
    }
  });

  $(document).on("click", function (e) {
    if (!$(e.target).closest("#search-input, #suggestions").length) {
      $suggestions.hide();
    }
  });

  // === 🔟 Если уже есть query в URL, применяем сразу ===
  if (currentPage === "courses.html") {
    const urlParams = new URLSearchParams(window.location.search);
    const queryFromURL = urlParams.get("query");
    if (queryFromURL) {
      $searchInput.val(queryFromURL);
      applySearch(queryFromURL.toLowerCase());
      sortVisibleCourses();
    }
  }

  // обновляем позицию при скролле/изменении размера
  positionSuggestions();
  $(window).on("resize scroll", positionSuggestions);
});













function initCounterUp(){
    const counters = qsa("[data-counter]");
    if(counters.length===0) return;
    function animateCounter(el){
        const target = parseInt(el.getAttribute("data-counter"),10) || 0;
        const duration = parseInt(el.getAttribute("data-counter-duration") || "1500",10);
        const suffix = el.getAttribute("data-counter-suffix") || "";
        let startTime = null;
        function tick(ts){
            if(!startTime) startTime = ts;
            const progress = Math.min((ts - startTime) / duration, 1);
            const valueNow = Math.floor(progress * target);
            el.textContent = valueNow.toLocaleString("en-US") + (progress===1 ? suffix : "");
            if(progress < 1){
                requestAnimationFrame(tick);
            }
        }
        requestAnimationFrame(tick);
    }
    if("IntersectionObserver" in window){
        const io = new IntersectionObserver((entries, obs)=>{
            entries.forEach(entry=>{
                if(entry.isIntersecting && !entry.target._animated){
                    entry.target._animated = true;
                    animateCounter(entry.target);
                    obs.unobserve(entry.target);
                }
            });
        },{threshold:0.4});
        counters.forEach(el=>io.observe(el));
    } else {
        counters.forEach(el=>animateCounter(el));
    }
}



function initCopyToClipboard(){
    const blocks = qsa(".copy-block");
    if(blocks.length===0) return;
    blocks.forEach(block=>{
        const btn = block.querySelector(".copy-btn");
        const tooltip = block.querySelector(".copy-tooltip");
        const textEl = block.querySelector(".copy-text");
        if(!btn || !textEl) return;
        btn.addEventListener("click", async ()=>{
            const text = textEl.innerText || textEl.textContent || "";
            try{
                await navigator.clipboard.writeText(text);
                btn.classList.add("copied");
                btn.setAttribute("disabled","true");
                if(tooltip){
                    tooltip.classList.add("show");
                    tooltip.textContent = "Скопировано!";
                }
                playBeep();
                setTimeout(()=>{
                    btn.classList.remove("copied");
                    btn.removeAttribute("disabled");
                    if(tooltip){
                        tooltip.classList.remove("show");
                        tooltip.textContent = "Скопировано!";
                    }
                },1500);
            }catch(err){
                if(tooltip){
                    tooltip.classList.add("show");
                    tooltip.textContent = "Ошибка копирования";
                    setTimeout(()=>tooltip.classList.remove("show"),1500);
                }
            }
        });
    });
}


function initLazyLoadImages(){
    const imgs = qsa("img[data-src]");
    if(imgs.length===0) return;
    function loadImg(img){
        if(img.dataset.loaded === "true") return;
        img.src = img.getAttribute("data-src");
        img.removeAttribute("data-src");
        img.dataset.loaded = "true";
        img.classList.add("loaded");
    }
    if("IntersectionObserver" in window){
        const observer = new IntersectionObserver((entries, obs)=>{
            entries.forEach(entry=>{
                if(entry.isIntersecting){
                    loadImg(entry.target);
                    obs.unobserve(entry.target);
                }
            });
        }, {root:null, rootMargin:"200px 0px", threshold:0});
        imgs.forEach(img=>observer.observe(img));
    } else {
        function check(){
            const vh = window.innerHeight;
            const top = window.scrollY || document.documentElement.scrollTop;
            const bottom = top + vh + 200;
            imgs.forEach(img=>{
                if(img.dataset.loaded === "true") return;
                const rect = img.getBoundingClientRect();
                const absoluteTop = rect.top + top;
                if(absoluteTop < bottom){
                    loadImg(img);
                }
            });
        }
        window.addEventListener("scroll", check);
        window.addEventListener("resize", check);
        check();
    }
}

function ensureScrollProgressBarElement(){
    let bar = qs("#scrollProgressBar");
    if(!bar){
        const wrap = document.createElement("div");
        wrap.className = "scroll-progress-wrapper";
        const inner = document.createElement("div");
        inner.id = "scrollProgressBar";
        inner.className = "scroll-progress-bar";
        wrap.appendChild(inner);
        document.body.appendChild(wrap);
        bar = inner;
    }
    return bar;
}


function initScrollProgressBar(){
    const bar = ensureScrollProgressBarElement();
    function updateBar(){
        const scrollTop = window.scrollY || document.documentElement.scrollTop;
        const docHeight = document.documentElement.scrollHeight - document.documentElement.clientHeight;
        const progress = docHeight > 0 ? (scrollTop / docHeight) * 100 : 0;
        bar.style.width = progress + "%";
    }
    updateBar();
    window.addEventListener("scroll", updateBar);
    window.addEventListener("resize", updateBar);
}

function showNotification(message, type = "info") {
    const existingNotifications = document.querySelectorAll(".notification");
    existingNotifications.forEach(notification => notification.remove());
    const notification = document.createElement("div");
    notification.className = `notification notification-${type}`;
    notification.textContent = message;
    document.body.appendChild(notification);
    if (!document.querySelector("#notification-styles")) {
        const style = document.createElement("style");
        style.id = "notification-styles";
        style.textContent = `
            .notification {
                position: fixed;
                top: 20px;
                right: 20px;
                padding: 15px 20px;
                border-radius: 5px;
                color: white;
                font-weight: bold;
                z-index: 1000;
                opacity: 0;
                transform: translateX(100%);
                transition: all 0.3s ease;
                max-width: 300px;
            }
            .notification-success { background-color: #4CAF50; }
            .notification-error { background-color: #f44336; }
            .notification-info { background-color: #2196F3; }
            .notification-warning { background-color: #ff9800; }
            .notification.show {
                opacity: 1;
                transform: translateX(0);
            }
            .spinner {
                display: inline-block;
                width: 16px;
                height: 16px;
                border: 2px solid #ffffff;
                border-radius: 50%;
                border-top-color: transparent;
                animation: spin 1s ease-in-out infinite;
                margin-right: 8px;
            }
            @keyframes spin {
                to { transform: rotate(360deg); }
            }
        `;
        document.head.appendChild(style);
    }
    setTimeout(() => notification.classList.add("show"), 100);
    setTimeout(() => {
        notification.classList.remove("show");
        setTimeout(() => notification.remove(), 300);
    }, 3000);
}



document.addEventListener("DOMContentLoaded", ()=>{
    initAccordion();
    initThemeToggle();
    initShowTime();
    initReadMore();
    initStars();
    initGalleryHero();
    initMenuKeyboardNav();
    initLoadMore();
    initSoundOnAction();

    initBackgroundChanger();
    initClockTicker();
    initScrollProgressBar();
    initCounterUp();
    initCopyToClipboard();
    initLazyLoadImages();

});
